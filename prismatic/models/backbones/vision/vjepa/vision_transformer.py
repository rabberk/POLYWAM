...
# --- Inlined from src/masks/utils.py ---
...
# --- Inlined from src/utils/tensors.py ---
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    with torch.no_grad():
        import math as _math
        l = (1. + _math.erf((a - mean) / (std * _math.sqrt(2.)))) / 2.
        u = (1. + _math.erf((b - mean) / (std * _math.sqrt(2.)))) / 2.
        tensor.uniform_(2 * l - 1, 2 * u - 1)
        tensor.erfinv_()
        tensor.mul_(std * _math.sqrt(2.))
        tensor.add_(mean)
