...
class DistributedOverwatch:
    def __init__(self, name: str) -> None:
        ...
        # Collectives get an explicit timeout, well above the 10-minute default.
        # Writing a multi-gigabyte checkpoint stalls whichever rank is doing the IO
        # for long enough that its peers declare it gone and abort the run -- twice
        # observed within seconds of a save completing. The timeout has to be set
        # here, at process-group creation: the NCCL environment variables do not
        # reach it, and a group's timeout cannot be changed afterwards.
        timeout = timedelta(seconds=int(os.environ.get("JEPA_WAM_DIST_TIMEOUT_SEC", 3600)))
        ...
        try:
            self.distributed_state = PartialState(timeout=timeout)
        except TypeError:
            # Older accelerate versions do not forward keyword arguments.
        ...
        self.logger.setLevel(
            logging.INFO
            if self.distributed_state.is_main_process or self.distributed_state.is_local_main_process
            else logging.ERROR
        )
